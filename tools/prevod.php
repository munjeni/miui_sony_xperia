...
preg_match_all('| <string name="(.*?)">(.*?)(</string>)|is', $a, $matchesa, PREG_SET_ORDER);
preg_match_all('| <string name="(.*?)">(.*?)</string>|is', $b, $matchesb, PREG_SET_ORDER);
...
for ($i=0; $i<count($matchesa); $i++) {
    ...
    if (isset($translated[ $matchesa[$i][1] ]["trim"])) {
        $out .= '    <string name="' . $translated[ $matchesa[$i][1] ]["orig"] . '">' . $translated[ $matchesa[$i][1] ]["string"] . "</string>\n";
    } else {
        $out .= '    <string name="' . $matchesa[$i][1] . '">' . $origin[ $matchesa[$i][1] ] . "</string>\n";
    }
}
